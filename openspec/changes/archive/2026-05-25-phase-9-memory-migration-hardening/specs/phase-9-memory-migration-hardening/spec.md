# Spec: Phase 9 - Memory Migration Hardening
...
| Scenario | Test File |
|----------|-----------|
| companion runtime learn event is accepted | `tests/memory/migration-contract.test.ts` |
| Malformed events do not crash reconstruction | `tests/memory/migration-contract.test.ts` |
| Events are ordered by timestamp | `tests/memory/ops-store.test.ts` |
| Latest learned fact wins | `tests/memory/reconstruct.test.ts` |
| Learned facts are included in prompt | `tests/prompt.test.ts` |
| Memory prompt layer is bounded | `tests/prompt.test.ts` |
| Boot reports reconstruction counts | `tests/cli-memory.test.ts` |
| Remember filters companion runtime learn events | `tests/cli-memory.test.ts` |
| Remember displays learned facts | `tests/cli-memory.test.ts` |

---

*Spec: Kintsugi - 2026-05-23*
