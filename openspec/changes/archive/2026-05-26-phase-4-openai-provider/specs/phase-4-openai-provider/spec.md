# Spec: Phase 4 — Provider Adapters
...
| Scenario | Test File |
|----------|-----------|
| OpenAI Chat streaming | `tests/openai-chat-provider.test.ts` (mocked HTTP) |
| OpenAI Responses streaming | `tests/openai-responses-provider.test.ts` (mocked HTTP) |
| Anthropic Messages streaming | `tests/anthropic-messages-provider.test.ts` (mocked HTTP) |
| Delta content mapping | `tests/provider-sse.test.ts` |
| Malformed line skipped | `tests/provider-sse.test.ts` |
| 401 auth failure | `tests/openai-chat-provider.test.ts` |
| API key not in errors | `tests/openai-chat-provider.test.ts`, `tests/anthropic-messages-provider.test.ts` |
| Default mock provider | `tests/provider-registry.test.ts`, `tests/cli-args.test.ts` |
| Missing API key | `tests/provider-registry.test.ts` |
