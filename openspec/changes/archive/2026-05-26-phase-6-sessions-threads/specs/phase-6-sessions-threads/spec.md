# Spec: Phase 6 — Sessions and Threads
...
| Scenario | Test File |
|----------|-----------|
| Events written on turn | `tests/session-store.test.ts` |
| Append-only writes | `tests/session-store.test.ts` |
| Index entry on start | `tests/session-index.test.ts` |
| Lists sessions | `tests/session-index.test.ts` |
| Resume restores conversation | `tests/session-replay.test.ts` |
| Resume missing session | `tests/session-replay.test.ts` |
| Malformed line skipped | `tests/session-replay.test.ts` |
| Export produces markdown | `tests/session-export.test.ts` |
