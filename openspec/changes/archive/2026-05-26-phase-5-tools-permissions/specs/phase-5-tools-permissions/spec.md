# Spec: Phase 5 — Tools and Permissions
...
| Scenario | Test File |
|----------|-----------|
| Tool spec JSON Schema | `tests/tools-registry.test.ts` |
| Lookup find/miss | `tests/tools-registry.test.ts` |
| Read inside roots | `tests/tool-read.test.ts` |
| Read outside roots | `tests/tool-read.test.ts` |
| Write with deny | `tests/tool-write.test.ts` |
| Bash requires approval | `tests/permissions.test.ts` |
| Explicit rule override | `tests/permissions.test.ts` |
| No rule falls to default | `tests/permissions.test.ts` |
